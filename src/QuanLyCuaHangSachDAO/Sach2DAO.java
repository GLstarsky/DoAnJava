...
public class Sach2DAO {
	...
	public Sach2DTO getSachById(int maSach) {
	    Sach2DTO sach = new Sach2DTO();
	    MySQL_connect mySQLConnect = new MySQL_connect();
	    Connection conn = mySQLConnect.connect();
	    try {
	        String sql = "SELECT sach.Ma, sach.Ten, sach.TacGia, sach.HinhAnh, phanloai.Ten AS PhanLoai, nhaxuatban.Ten AS NXB, sach.SoLuong, sach.DonGia FROM sach, nhaxuatban, phanloai WHERE sach.Ma = ? ";
	        PreparedStatement pre = conn.prepareStatement(sql);
	        pre.setInt(1, maSach);
	        ResultSet rs = pre.executeQuery();
	        if (rs.next()) {
	...
	public void updateGiaBanMoi(int maSach, int giaNhapVao) {
	    Sach2DTO sach = new Sach2DTO();
	    MySQL_connect mySQLConnect = new MySQL_connect();
	    Connection conn = mySQLConnect.connect();
	    try {
	        String sql = "UPDATE sach SET DonGia = ? WHERE Ma = ?";
	        PreparedStatement pre = conn.prepareStatement(sql);
	        int giaBanMoi = giaNhapVao + (int)(Math.round(15.0 / 100 * giaNhapVao)); // Sử dụng 15.0
	        pre.setInt(1, giaBanMoi);
	        pre.setInt(2, maSach);
	        int rowsAffected = pre.executeUpdate();
	        if (rowsAffected > 0) {
	        } else {
